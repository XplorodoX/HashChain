...
struct Point {
    ...
    // Parameterloser Konstruktor, der z. B. benötigt wird, wenn Point
...
// was insbesondere in HashChain und HashOpen benötigt wird.
...
// Zweite Streuwertfunktion für Punkte p bei Tabellengröße n.
uint hashval2 (Point p, uint n) {
    // Damit die Werte h immer teilerfremd zur Tabellengröße n sind,
    // wird immer eine ungerade Zahl von 1 bis n-1 geliefert.
    // Dann können sowohl Zweierpotenzen als auch Primzahlen als
    // Tabellengrößen verwendet werden.
...
// Durch diese Definition können Point-Objekte mit dem normalen
...
void test (H tab, bool prompt) {
    ...
    // und entsprechende Operationen auf der Tabelle ausführen.
    string cmd, name;
    int x, y;
    while (true) {
	if (prompt) cout << "cmd: ";
...
// Hauptprogramm.
// Das erste Kommandozeilenargument ist die Tabellengröße n,
